...
def deploy(
    stack: Stack, template_body: str, parameters: Dict, tags: Dict, wait: bool = True,
):
    ...
    try:
        if stack.exists:
            ...
            method = stack.cloudformation.update_stack
        else:
            logger.debug("Stack does not exist - setting method to create_stack")
            method = stack.cloudformation.create_stack  # type: ignore
